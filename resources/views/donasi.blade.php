                    ...
                    <table class="table table-hover align-middle mb-0" id="tabelDonasi">
                        <thead>
                            <tr style="height: 50px;">
                                ...
                                <th>Nama Program</th>
                                <th class="text-center cursor-pointer" id="sortMulai">Mulai <i
                                        class="bi bi-arrow-down-up small text-muted sort-icon"></i></th>
                                <th class="text-center cursor-pointer" id="sortSelesai">Selesai <i
                                        class="bi bi-arrow-down-up small text-muted sort-icon"></i></th>
                ...
                <div class="modal-body p-4 bg-white">
                    <form id="formDonasi" enctype="multipart/form-data">
                        ...
                        <div class="donation-card-wrapper">
                            ...
                            {{-- Upload Foto --}}
                            <div class="mb-3">
                                <label class="form-label fw-bold text-success small text-uppercase ls-1">Poster / Gambar
                                    (Max 2 Mb)</label>
                            ...
                            <div class="mb-3">
                                <label class="form-label fw-bold text-success small text-uppercase ls-1">Nama
                                    Program</label>
                                <input type="text" class="form-control rounded-pill-input" name="nama_donasi"
                                    id="nama_donasi" placeholder="Misal: Renovasi Masjid" required>
                            </div>

                            {{-- Target Dana --}}
                            <div class="mb-3">
                                <label class="form-label fw-bold text-success small text-uppercase ls-1">Target
                                    Dana</label>
                                <div class="input-group rounded-pill-group">
                                    <span
                                        class="input-group-text bg-white border-end-0 text-success fw-bold ps-3">Rp</span>
                                    <input type="text" class="form-control border-start-0 ps-1 fw-bold text-dark"
                                        id="display_target_dana" placeholder="0" required>
                            ...
                            <div class="row g-2 mb-3">
                                <div class="col-6">
                                    ...
                                    <input type="date" class="form-control rounded-pill-input" name="tanggal_mulai"
                                        id="tanggal_mulai" required value="{{ date('Y-m-d') }}">
                                </div>
                                <div class="col-6">
                                    <label
                                        class="form-label fw-bold text-success small text-uppercase ls-1">Selesai</label>
                                    <input type="date" class="form-control rounded-pill-input" name="tanggal_selesai"
                                        id="tanggal_selesai">
                                </div>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <textarea class="form-control rounded-box-input" name="deskripsi" id="deskripsi" rows="2"
            ...
            <div class="modal-content border-0 shadow-lg modal-rounded">
                ...
                <div class="modal-body p-4">
                    <div class="row g-3 mb-4">
                        <div class="col-md-4">
                            <div class="stat-card bg-light-green"><small class="text-success fw-bold text-uppercase ls-1"
                                    style="font-size: 0.7rem;">Target</small>
                                <h5 class="fw-bold text-dark mt-1 mb-0" id="detTarget">Rp 0</h5>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="stat-card bg-gradient-green text-white shadow-sm"><small
                                    class="text-white-50 fw-bold text-uppercase ls-1"
                                    style="font-size: 0.7rem;">Terkumpul</small>
                                <h5 class="fw-bold mt-1 mb-0" id="detTerkumpul">Rp 0</h5>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="stat-card bg-light-red"><small class="text-danger fw-bold text-uppercase ls-1"
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3 px-1">
                        ...
                        <button class="btn btn-sm btn-outline-success rounded-pill px-3 fw-bold" data-bs-toggle="modal"
                            data-bs-target="#modalInputPemasukan"><i class="bi bi-plus-lg me-1"></i> Input Manual</button>
                    </div>
                        ...
                        <div class="donation-card-wrapper p-3">
                            <div class="mb-3">
                                ...
                                <input type="text"
                                    ...
                                    style="font-size: 1.1rem;" id="display_nominal" placeholder="0" required>
                                <input type="hidden" name="nominal" id="real_nominal">
                            </div>
                            <div class="mb-2">
                                <label class="form-label fw-bold text-success small">Nama Donatur</label>
                                <input type="text" class="form-control rounded-pill-input form-control-sm"
                                    name="nama_donatur" placeholder="Hamba Allah" required>
                            </div>
                            <div class="mb-2">
                                <label class="form-label fw-bold text-success small">Metode</label>
                                <select class="form-select rounded-pill-input form-select-sm" name="metode_pembayaran"
                                    required>
                            ...
                            <div class="mb-2">
                                ...
                                <input type="date" class="form-control rounded-pill-input form-control-sm"
                                    name="tanggal" value="{{ date('Y-m-d') }}" required>
                            </div>
                            <div class="mb-3">
                                <textarea class="form-control rounded-box-input form-control-sm" name="pesan" rows="2"
                                    placeholder="Pesan / Doa (Opsional)"></textarea>
                            </div>
                            <button type="submit"
                                class="btn btn-gradient-green w-100 rounded-pill fw-bold shadow-sm py-2">Simpan
                                Data</button>
                        </div>
                    </form>
                </div>
    ...
    @push('scripts')
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                const formatTanggal = (str) => !str ? '-' : new Date(str).toLocaleDateString('id-ID', {
                    day: '2-digit',
                    month: 'short',
                    year: 'numeric'
                });
                ...
                window.editDonasi = async (id) => {
                    try {
                        ...
                        if (data.tanggal_selesai) document.getElementById('tanggal_selesai').value = data
                ...
                function renderTable(data, startNum) {
                    ...
                    if (data.length === 0) {
                        tbody.innerHTML =
                            `<tr><td colspan="${colCount}" class="text-center py-4 text-muted">Belum ada program donasi.</td></tr>`;
                        return;
                    }
                    ...
                    data.forEach((item, i) => {
                        ...
                        const isExpired = item.tanggal_selesai && new Date(item.tanggal_selesai) < new Date()
                            .setHours(0, 0, 0, 0);
                ...
                document.getElementById('sortMulai').addEventListener('click', () => {
                    state.sortBy = 'tanggal_mulai';
                    state.sortDir = state.sortDir === 'asc' ? 'desc' : 'asc';
                    loadDonasi();
                });
                document.getElementById('sortSelesai').addEventListener('click', () => {
                    state.sortBy = 'tanggal_selesai';
                    state.sortDir = state.sortDir === 'asc' ? 'desc' : 'asc';
                    loadDonasi();
                });

                function updateSortIcons() {
                    document.querySelectorAll('.sort-icon').forEach(i => i.className =
                        'bi bi-arrow-down-up small text-muted sort-icon');
                    let active = state.sortDir === 'asc' ? 'bi bi-arrow-up text-primary' :
                        'bi bi-arrow-down text-primary';
                    if (state.sortBy === 'tanggal_mulai') document.querySelector('#sortMulai i').className =
                        `${active} small sort-icon`;
                    if (state.sortBy === 'tanggal_selesai') document.querySelector('#sortSelesai i').className =
                        `${active} small sort-icon`;
                }
                ...
                function renderPagination(response) {
                    ...
                    nav.querySelectorAll('a.page-link').forEach(a => a.addEventListener('click', (e) => {
                        ...
                        if (a.dataset.url && a.dataset.url !== 'null') {
                            state.page = new URLSearchParams(a.dataset.url.split('?')[1]).get('page');
                            loadDonasi();
                        }
                    }));
                }
                ...
                window.hapusDonasi = async (id) => {
                    ...
                    if (c.isConfirmed) {
                        await fetch(`/pengurus/donasi/${id}`, {
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': token
                            }
                ...
                async function refreshModalDetail() {
                    ...
                    if (data.pemasukan.length === 0) {
                        tbodyRiwayat.innerHTML =
                            `<tr><td colspan="6" class="text-center text-muted">Belum ada donasi masuk.</td></tr>`;
                    } else {
                        data.pemasukan.forEach(p => {
                            let badgeMetode = p.metode_pembayaran === 'tunai' ?
                                '<span class="badge bg-secondary">Tunai</span>' :
                                '<span class="badge bg-primary">Transfer</span>';
                            let badgeStatus = p.status === 'success' ?
                                '<span class="badge bg-success bg-opacity-10 text-success border border-success">Berhasil</span>' :
                                (p.status === 'pending' ?
                ...
                window.hapusPemasukan = async (id) => {
                    ...
                    if (c.isConfirmed) {
                        await fetch(`/pengurus/pemasukan-donasi/${id}`, {
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': token
                            }
