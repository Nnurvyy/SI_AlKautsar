...
@section('content')
    ...
    @if ($programDonasi->isEmpty())
        <div class="container mb-5">
            <div class="alert text-center py-5 rounded-4 shadow-sm border-0"
                        ...
                        @foreach ($programDonasi as $program)
                            <div class="swiper-slide">
                                <div class="donation-slide">
                                    <img src="{{ $program->gambar_url }}" class="donation-slide-img"
                                        alt="{{ $program->nama_donasi }}">
                                    <div class="donation-slide-overlay">
                                        <h5 class="fw-bold">{{ $program->nama_donasi }}</h5>
                                        <div class="d-flex justify-content-between small mb-1">
                                            <span>Terkumpul: Rp
                                                {{ number_format($program->dana_terkumpul, 0, ',', '.') }}</span>
                                        ...
                                        <button onclick="openDonasiModal('{{ $program->id_donasi }}')"
                                            class="btn btn-light btn-sm w-100 rounded-pill fw-bold text-success">Lihat
                                            Detail</button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-7">
                                <div class="card-body">
                                    <div>
                                        ...
                                        <div class="mb-2">
                                            @if ($program->tanggal_selesai)
                                                @php
                                                    $sisa = \Carbon\Carbon::now()
                                                        ->startOfDay()
                                                        ->diffInDays(\Carbon\Carbon::parse($program->tanggal_selesai)->startOfDay(), false);
                                                @endphp
                                                @if ($sisa > 0)
                                                    <small class="text-warning fw-bold"><i class="bi bi-clock"></i> Sisa
                                                        {{ ceil($sisa) }} hari</small>
                                                @elseif($sisa == 0)
                                                    <small class="text-danger fw-bold"><i
                                                            class="bi bi-exclamation-circle"></i> Berakhir Hari Ini</small>
                                                @else
                                                    <small class="text-muted fw-bold"><i class="bi bi-x-circle"></i>
                                                        Berakhir</small>
                                                @endif
                                            @else
                                                <small class="text-success fw-bold"><i class="bi bi-infinity"></i>
                                                    Unlimited</small>
                                            @endif
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <div class="text-end">
                                            <button onclick="openDonasiModal('{{ $program->id_donasi }}')"
                                                class="btn-detail-donasi">Lihat Detail</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="modal-dialog modal-narrow">
            <div class="modal-content border-0 shadow rounded-4 overflow-hidden">
                ...
                <div class="position-relative bg-dark">
                    <button type="button"
                        class="btn-close position-absolute top-0 end-0 m-3 bg-white p-2 rounded-circle shadow-sm"
                        style="z-index: 10; opacity: 1;" data-bs-dismiss="modal"></button>
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="mb-4">
                        <div class="d-flex justify-content-between align-items-end mb-1">
                            ...
                            <div class="text-end">
                                ...
                                <span class="small text-muted">Target: <span id="mTarget" class="fw-bold">Rp
                                        0</span></span>
                            </div>
                        </div>
                        <div class="progress" style="height: 10px; border-radius: 5px;">
                            <div id="mProgress" class="progress-bar bg-success" role="progressbar" style="width: 0%">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="d-flex flex-column">
                            ...
                            <form id="formDonasiOnline">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" inputmode="numeric"
                                            class="form-control border-start-0 input-nominal" id="pay_nominal"
                                            name="nominal" placeholder="Minimal 10.000" autocomplete="off" required>
                                    </div>
                                    <small class="text-danger d-none" id="nominalError"
                                        style="font-size: 0.75rem;">Minimal Rp 10.000</small>
                                </div>
                                ...
                                @auth('jamaah')
                                    <div class="alert alert-success py-2 px-3 mb-3 d-flex align-items-center">
                                        ...
                                        @else
                                            <div class="rounded-circle bg-white text-success d-flex align-items-center justify-content-center me-2 fw-bold"
                                        ...
                                        <div>
                                            <small class="d-block text-muted"
                                                style="font-size: 0.7rem; line-height: 1;">Berdonasi sebagai:</small>
                                            <span
                                                class="fw-bold text-dark small">{{ Auth::guard('jamaah')->user()->name }}</span>
                                        </div>
                                    </div>
                                @else
                                    <div class="mb-2">
                                        <input type="text" class="form-control mb-2" id="pay_nama" name="nama"
                                ...
                                <div class="mb-3">
                                    <textarea class="form-control" id="pay_pesan" name="pesan" rows="2"
                                        placeholder="Tulis doa atau dukungan (Opsional)"></textarea>
                                </div>
                        ...
                        <div>
                            ...
                            <p class="text-muted small mb-1 desc-clamped" id="mDeskripsi"
                                style="white-space: pre-line; line-height: 1.6;">Memuat deskripsi...</p>
                            ...
                            <div class="text-center">
                                <button id="btnLoadMore"
                                    class="btn btn-sm btn-outline-secondary rounded-pill d-none px-4">Lihat Lebih
                                    Banyak</button>
                            </div>
                        </div>
                ...
                .then(data => {
                    if (currentDonaturPage === 1) {
                        ...
                        imgEl.onerror = function() {
                            this.src = '/images/donasi/default.jpg';
                        };

                        document.getElementById('mNama').textContent = data.nama_donasi;
                        document.getElementById('mTerkumpul').textContent = 'Rp ' + new Intl.NumberFormat('id-ID')
                            .format(data.terkumpul);
                        document.getElementById('mTarget').textContent = 'Rp ' + new Intl.NumberFormat('id-ID').format(
                            data.target_dana);
                    ...
                    if (data.donatur.data.length > 0) {
                        data.donatur.data.forEach(d => {
                            if (d.pesan && d.pesan.trim() !== "" && d.pesan !== "-") {
                                ...
                                if (d.avatar_url && !d.avatar_url.includes('default-user.png')) {
                                    avatarHtml =
                                        `<img src="${d.avatar_url}" class="rounded-circle" style="width: 40px; height: 40px; object-fit: cover; margin-right: 12px;">`;
                                } else {
                                    avatarHtml =
                                        `<div class="donatur-avatar" style="width: 40px; height: 40px; background: #e2e8f0; color: #64748b; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 1.1rem; margin-right: 12px;">${nama.charAt(0).toUpperCase()}</div>`;
                                }
                        ...
                        listContainer.innerHTML =
                            '<div class="text-center small text-muted py-3">Belum ada doa/dukungan. Jadilah yang pertama!</div>';
                    }
