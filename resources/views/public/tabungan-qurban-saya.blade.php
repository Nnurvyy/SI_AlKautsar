...
@section('content')
    ...
    @if (!Auth::guard('jamaah')->check())
        ...
        <div class="modal fade show" id="modalLoginRequired" tabindex="-1" aria-modal="true" role="dialog"
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content text-center p-4 border-0 shadow rounded-4">
                    ...
                    <p class="text-muted mb-4">Fitur Tabungan Qurban hanya dapat diakses oleh Jamaah yang sudah masuk
                        (Login).</p>
                    <div class="d-grid gap-2">
                        <a href="{{ route('login') }}" class="btn btn-success btn-lg fw-semibold">Login Sekarang</a>
                        <a href="{{ route('public.landing') }}" class="btn btn-outline-secondary btn-lg fw-semibold">Kembali
                            ke Beranda</a>
                    </div>
                </div>
            </div>
        ...
        <div class="container pt-4 pb-5 py-lg-5">
            ...
            <div class="row align-items-center mb-4">
                ...
                <div class="col-md-4 text-md-end d-none d-md-block">
                    <button class="btn btn-success rounded-pill px-4 shadow-sm" data-bs-toggle="modal"
                        data-bs-target="#modalBukaTabungan">
            ...
            @if ($tabungans->isEmpty())
                <div class="text-center py-5">
                    ...
                    <button class="btn btn-outline-success rounded-pill mt-2" data-bs-toggle="modal"
                        data-bs-target="#modalBukaTabungan">
                        ...
                        <div class="col-lg-6">
                            <div class="card card-tabungan h-100">
                                <div class="card-body p-4">
                                    ...
                                    <div class="d-flex justify-content-between align-items-start mb-3">
                                        <div>
                                            <a href="javascript:void(0)"
                                                onclick="openDetailModal('{{ $t->id_tabungan_hewan_qurban }}')"
                                            ...
                                            <small
                                                class="text-muted">{{ \Carbon\Carbon::parse($t->created_at)->format('d M Y') }}</small>
                                        </div>
                                        ...
                                        @else
                                            <span
                                                class="status-badge bg-secondary text-white">{{ ucfirst($t->status) }}</span>
                                        @endif
                                    </div>
                                    ...
                                    @if ($t->status == 'menunggu')
                                        <div
                                            class="alert alert-warning py-2 px-3 small mb-3 border-0 bg-warning bg-opacity-10 text-warning-emphasis">
                                    ...
                                    <div class="bg-light p-3 rounded mb-3">
                                        <h6 class="small fw-bold text-muted text-uppercase mb-2"
                                            style="letter-spacing: 1px;">Rencana Qurban</h6>
                                        @foreach ($t->details as $detail)
                                            <div class="list-hewan-item d-flex justify-content-between">
                                                <span>
                                                    <strong>{{ $detail->jumlah_hewan }}</strong> ekor
                                                    {{ ucfirst($detail->hewan->nama_hewan) }}
                                                    <small
                                                        class="text-muted">({{ ucfirst($detail->hewan->kategori_hewan) }})</small>
                                                </span>
                                            </div>
                                        @endforeach
                                    </div>
                                    ...
                                    <div class="mb-2 d-flex justify-content-between small fw-bold">
                                        <span class="text-success">Terkumpul: Rp
                                    ...
                                    <div class="row g-2">
                                        <div class="col-8">
                                            ...
                                            @if ($t->status == 'disetujui' && $sisa > 0)
                                                <button class="btn btn-success w-100 fw-bold" {{-- Pass Cicilan Per Bulan & Tipe ke Fungsi JS --}}
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content border-0 shadow">
                        ...
                        <form id="formCreateTabungan">
                            ...
                            <div id="hewanContainer">
                                <div class="hewan-row row g-2 align-items-center">
                                    <div class="col-8">
                                        ...
                                        <select class="form-select select-hewan" required
                                            onchange="calculateGrandTotal()">
                                            <option value="" data-harga="0">-- Pilih Hewan --</option>
                                            @foreach ($masterHewan as $h)
                                                <option value="{{ $h->id_hewan_qurban }}"
                                                    data-harga="{{ $h->harga_hewan }}">
                                                    {{ ucfirst($h->nama_hewan) }} {{ ucfirst($h->kategori_hewan) }} - Rp
                                                    {{ number_format($h->harga_hewan, 0, ',', '.') }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-3">
                                        ...
                                        <input type="number" class="form-control input-qty" value="1"
                                            min="1" required oninput="calculateGrandTotal()">
                                    </div>
                                    <div class="col-1 d-flex align-items-end">
                                        <button type="button" class="btn btn-danger btn-sm w-100 mt-4 btn-remove"
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6" id="divDuration">
                                    ...
                                    <input type="number" name="duration_months" class="form-control" value="12"
                                        min="1">
                                    <div class="form-text small">Est. cicilan: <span id="estCicilan"
                                            class="fw-bold text-dark">-</span> /bulan (Dibulatkan)</div>
                                </div>
                            </div>
                        </form>
                    </div>
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-success px-4" onclick="submitTabungan()">Simpan
                            Tabungan</button>
                    </div>
                </div>
            </div>
                        ...
                        <div class="box-payment mb-4">
                            <div class="d-flex align-items-center mb-3">
                                @if (Auth::guard('jamaah')->user()->avatar)
                                    <img src="{{ asset('storage/' . Auth::guard('jamaah')->user()->avatar) }}"
                                        class="rounded-circle me-3 border"
                                        style="width: 50px; height: 50px; object-fit: cover;">
                                @else
                                    <div class="rounded-circle bg-white text-success d-flex align-items-center justify-content-center me-3 fs-4 fw-bold border"
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" inputmode="numeric"
                                            class="form-control border-start-0 input-nominal" id="pay_nominal_tabungan"
                                            name="nominal" placeholder="Contoh: 100.000" required>
                                    </div>
                                    ...
                                    <div class="card border-0 shadow-sm h-100">
                                        <div class="card-body">
                                            ...
                                            <div class="alert alert-info py-2 px-3 small mb-0 mt-2">
                                                <i class="bi bi-calendar-check me-1"></i> Mulai: <span
                                                    id="dTglMulai">-</span>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="row g-2 text-center">
                                                <div class="col-4">
                                                    <div class="p-2 rounded bg-light border">
                                                        ...
                                                        <span class="d-block fw-bold text-dark small" id="dTarget">Rp
                                                            0</span>
                                                    </div>
                                                </div>
                                                <div class="col-4">
                                                    <div
                                                        ...
                                                        <span class="d-block fw-bold text-success small"
                                                            id="dTerkumpul">Rp 0</span>
                                                    </div>
                                                </div>
                                                <div class="col-4">
                                                    <div class="p-2 rounded bg-danger bg-opacity-10 border border-danger">
                                                        <small class="d-block text-danger"
                                                            style="font-size: 0.7rem;">Kurang</small>
                                                        <span class="d-block fw-bold text-danger small" id="dSisa">Rp
                                                            0</span>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="mt-3">
                                                ...
                                                <div class="progress" style="height: 8px;">
                                                    <div id="dProgressBar" class="progress-bar bg-success"
        ...
        function openDetailModal(id) {
            ...
            fetch(`/qurban-saya/${id}`)
                ...
                .then(data => {
                    ...
                    document.getElementById('detailId').innerText = 'ID: #' + data.id_tabungan_hewan_qurban.substring(0,
                        8);

                    const tglMulai = new Date(data.created_at).toLocaleDateString('id-ID', {
                        day: 'numeric',
                        month: 'long',
                        year: 'numeric'
                    });
                    ...
                    document.getElementById('dTarget').innerText = "Rp " + new Intl.NumberFormat('id-ID').format(
                    target);
                    document.getElementById('dTerkumpul').innerText = "Rp " + new Intl.NumberFormat('id-ID').format(
                        terkumpul);
                    ...
                    if (data.saving_type === 'cicilan') {
                        ...
                        document.getElementById('dCicilan').innerText = "Rp " + new Intl.NumberFormat('id-ID').format(
                            estCicilan) + " /bln";
                    } else {
                    ...
                    if (txList.length === 0) {
                        tbody.innerHTML =
                            `<tr><td colspan="4" class="text-center py-3 text-muted small">Belum ada riwayat transaksi.</td></tr>`;
                    } else {
