    ...
    <nav class="top-navbar navbar navbar-expand-lg bg-white border-bottom shadow-sm fixed-top">
        <div class="container-fluid px-4">
            ...
            <div class="d-flex align-items-center">
                <button
                    class="btn btn-light rounded-circle shadow-sm border me-3 d-flex justify-content-center align-items-center"
                    id="sidebar-toggle" type="button" style="width: 40px; height: 40px;">
                    <i class="bi bi-list fs-5"></i>
                </button>
                <div>
                    <h5 class="fw-bold mb-0 text-dark responsive-title">@yield('title')</h5>
                    <small
                        class="text-muted d-none d-sm-block">{{ now()->locale('id')->translatedFormat('l, d F Y') }}</small>
                </div>
            </div>

            <ul class="navbar-nav ms-auto d-flex flex-row align-items-center gap-3">
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle d-flex align-items-center gap-2 p-1 rounded-pill border bg-light pe-3"
                        href="#" role="button" data-bs-toggle="dropdown">
                        @if (Auth::user()->avatar)
                            <img src="{{ Auth::user()->avatar_url }}" class="rounded-circle object-fit-cover"
                                style="width: 32px; height: 32px;">
                        @else
                            <div class="rounded-circle bg-gradient-green text-white d-flex align-items-center justify-content-center shadow-sm"
                    ...
                    <ul class="dropdown-menu dropdown-menu-end shadow-lg border-0 rounded-4 mt-2 p-2">
                        <li>
                            <h6 class="dropdown-header text-uppercase small fw-bold text-muted ls-1">Akun Saya</h6>
                        </li>
                        ...
                        {{-- Menu Logout --}}
                        <li>
                            <a class="dropdown-item rounded-2 py-2 text-danger fw-bold" href="#"
