document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadKhotib() {
        ...
        const url = `/pengurus/khotib-jumat-data?page=${state.currentPage}&status=${state.status}&search=${state.search}&perPage=${state.perPage}&sortBy=${state.sortBy}&sortDir=${state.sortDir}`;
    ...
    window.editKhotib = async function(id_khutbah) {
        try {
            const res = await fetch(`/pengurus/khotib-jumat/${id_khutbah}`);
